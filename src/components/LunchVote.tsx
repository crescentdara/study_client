...
export default function LunchVote({ nickname, theme }: { nickname: string; theme: 'vscode' | 'excel' }) {
    ...
    const canVote = data.menus.length >= 3 && !data.myVoteMenuId;
    if (!excel) return <section className="code-block" style={{ margin: '12px 0 0', borderRadius: 0, border: 'none', borderTop: `1px solid ${colors.line}`, borderBottom: `1px solid ${colors.line}`, color: colors.ink }}>
        <div className="c-line"><span className="ln" onClick={() => setOpen((value) => !value)} style={{ cursor: 'pointer' }}>{open ? '▾' : '▸'}</span><span className="c-line-body" onClick={() => setOpen((value) => !value)} style={{ cursor: 'pointer' }}><span className="kw">const </span><span className="var">todayLunch</span><span className="pct">: </span><span className="typ">LunchMenu</span><span className="pct">{open ? '[] = [' : `[] // ${data.menus.length} menus`}</span><span className="cmt">  // {data.date || 'today'} · {data.voterCount} votes</span></span></div>
        {open && <>
        {data.menus.length === 0 ? <div className="c-line"><span className="ln">·</span><span className="c-line-body"><span className="cmt">// 메뉴를 한 줄 등록해 보세요.</span></span></div> : data.menus.map((item) => { const mine = item.nickname.trim().toLowerCase() === nickname.trim().toLowerCase(); return <div className="c-line" key={item.id}><span className="ln">{item.winner ? '👑' : '·'}</span><span className="c-line-body" style={{ display: 'flex', alignItems: 'center', gap: 5 }}><span className="pct">{'{ menu: '}</span><b className="str">"{item.menu}"</b><span className="pct">{', by: '}</span><span className="var">{item.nickname}</span><span className="pct">{', votes: '}</span><span className="num">{item.votes}</span><span className="pct">{' },'}</span><button type="button" onClick={() => void request('/api/lunch/votes', { menuId: item.id })} disabled={!canVote || mine} className="btn-secondary" style={{ marginLeft: 4, padding: '1px 6px', fontSize: 10 }}>{data.myVoteMenuId === item.id ? 'voted' : mine ? 'my menu' : data.menus.length < 3 ? 'need 3 menus' : 'vote()'}</button></span></div>})}
        <div className="c-line"><span className="ln">+</span><span className="c-line-body" style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span className="pct">{'{ menu: "'}</span><input value={menu} onChange={(event) => setMenu(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void request('/api/lunch/menus', { menu }); }} maxLength={100} placeholder="메뉴 입력" style={{ width: 130, border: 'none', borderBottom: '1px solid #4ec9b0', background: 'transparent', color: '#ce9178', outline: 'none', font: 'inherit', fontSize: 12, padding: '1px 2px' }} /><span className="pct">{'" },'}</span><button type="button" onClick={() => void request('/api/lunch/menus', { menu })} className="btn-secondary" style={{ padding: '1px 6px', fontSize: 10 }}>register()</button></span></div>
    ...
    return <section style={{ margin: 0, borderTop: `1px solid ${colors.line}`, borderLeft: `1px solid ${colors.line}`, background: colors.paper, color: colors.ink }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '30px minmax(110px,1fr) 84px 48px 54px' }}>
            ...
            <div style={{ ...cell, justifyContent: 'center', background: '#f8fbf7' }}>＋</div><div style={{ ...cell, padding: 0 }}><input value={menu} onChange={(event) => setMenu(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void request('/api/lunch/menus', { menu }); }} maxLength={100} placeholder="새 메뉴 입력" style={{ width: '100%', border: 0, outline: 'none', padding: '6px 7px', color: colors.ink, background: '#fff', fontSize: 12 }} /></div><div style={{ ...cell, gridColumn: 'span 2', color: colors.mute }}>내 추천 메뉴</div><div style={{ ...cell, justifyContent: 'center' }}><button type="button" onClick={() => void request('/api/lunch/menus', { menu })} style={{ border: 'none', background: 'transparent', color: colors.accent, fontWeight: 800, cursor: 'pointer', fontSize: 11 }}>등록</button></div>
        </div>
