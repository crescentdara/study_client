...
function Lobby({ nickname, emoji, sessionId, onNicknameChange, onEmojiChange, onJoinRoom }: LobbyProps) {
  ...
  const handleTermCmd = (cmd: string) => {
    ...
    if (!trimmed) return;
    const next: { type: "cmd" | "out" | "err"; text: string }[] = [
      ...termHistory,
  ...
  const saveProfile = () => {
    ...
    if (!nextName) { setError("Nickname is required."); return; }
      ...
      <div style={{
        ...
        {activePanel === "explorer" && (
          <div style={{ flex: 1, overflow: "auto", display: "flex", flexDirection: "column" }}>
            ...
            {/* USER section */}
            <div
              style={{ display: "flex", alignItems: "center", gap: "5px", padding: "3px 10px", fontSize: "11px", color: "#bbb", cursor: "pointer", userSelect: "none" }}
            ...
            {userExpanded && (
              <div style={{ padding: "2px 0 8px", flexShrink: 0 }}>
                ...
                <div
                  style={{ padding: "2px 14px 2px 28px", fontSize: "11px", color: "#569cd6", cursor: "pointer" }}
            ...
            {roomsExpanded && (
              <>
                  ...
                  {rooms.map((room) => (
                    <div
                      ...
                      style={{
                        ...
                        fontSize: "12px",
                        background: hoveredRoom === room.roomId ? "rgba(255,255,255,0.06)" : "transparent",
                        cursor: "pointer",
                        gap: "5px",
                      }}
                ...
                <div style={{ display: "flex", gap: "4px", padding: "5px 8px", borderTop: "1px solid #3e3e42", flexShrink: 0 }}>
                  <button className="btn-secondary" style={{ fontSize: "10px", padding: "2px 8px" }} onClick={fetchRooms} disabled={loading}>
                    {loading ? "…" : "↺"}
                  </button>
                  <button
                    className={showCreate ? "btn-primary" : "btn-secondary"}
                    style={{ flex: 1, fontSize: "10px", padding: "2px 0", background: showCreate ? "#0e639c" : "#00ab77", color: "#fff" }}
        ...
        <div style={{ flex: 1, overflow: "auto" }}>
          ...
          {!showCreate && (
            <div className="code-block" style={{ borderRadius: 0, border: "none" }}>
              ...
              {L(<><span className="kw">import </span><span className="pct">{"{ "}</span><span className="var">Room</span><span className="pct">{" }"}</span><span className="kw"> from </span><span className="str">"@study/platform"</span></>)}
              {L(<></>)}
              {L(<><span className="kw">const </span><span className="var">rooms</span><span className="pct">: </span><span className="typ">Room</span><span className="pct">[] = [</span><span className="cmt">{`  // ${rooms.length} room${rooms.length !== 1 ? "s" : ""}`}</span></>)}

              {rooms.length === 0 && L(<><span className="cmt">{"  // no rooms — create one!"}</span></>)}

              {rooms.map((room) => {
                const opt =
                  room.studyType === "BASEBALL" ? `${room.digits}digit` :
                  room.studyType === "TETRIS" ? "20x10" :
                  room.studyType === "OMOK" ? "19x19" :
                  room.studyType === "OLDMAID" ? `${room.maxPlayers}p` :
                  `${room.boardSize}x${room.boardSize}`;
                const lineNum = ln++;
                return (
                  <div className="c-line" key={room.roomId}>
                    <span className="ln">{lineNum}</span>
                    <span className="c-line-body" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%", gap: "8px" }}>
                      <span style={{ flex: 1, overflow: "hidden" }}>
                        {"  "}
                        <span className="pct">{"{ "}</span>
                        <span className="var">name</span><span className="pct">: </span><span className="str">"{room.roomName}"</span>
                        <span className="pct">, </span>
                        <span className="var">type</span><span className="pct">: </span><span className="typ">{room.studyType}</span>
                        <span className="pct">, </span>
                        <span className="var">opt</span><span className="pct">: </span><span className="str">"{opt}"</span>
                        <span className="pct">, </span>
                        <span className="var">n</span><span className="pct">: </span><span className="num">{room.playerCount}</span><span className="pct">/</span><span className="num">{room.maxPlayers}</span>
                        <span className="pct">{" }"}</span>
                      </span>
                      <button className="btn-primary" style={{ fontSize: "11px", padding: "2px 10px", flexShrink: 0, marginRight: "4px" }} onClick={() => handleJoin(room.roomId)}>
                        .join()
                      </button>
                    </span>
                  </div>
          ...
          {showCreate && (
            <div className="code-block" style={{ borderRadius: 0, border: "none" }}>
              ...
              {L(<><span className="kw">import </span><span className="pct">{"{ "}</span><span className="var">StudyType</span><span className="pct">{" }"}</span><span className="kw"> from </span><span className="str">"@study/types"</span></>)}
              {L(<></>)}
              {L(<><span className="kw">async function </span><span className="fn">createRoom</span><span className="pct">{"() {"}</span></>)}

              {L(
                <span style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  <span className="kw">const </span><span className="var">name</span><span className="pct"> = </span>
                  <input
                    autoFocus
              ...
              {L(
                <span style={{ display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap" }}>
                  <span className="kw">const </span><span className="var">type</span><span className="pct">: </span><span className="typ">StudyType</span><span className="pct"> = </span>
                  {(["BASEBALL", "BINGO", "OMOK", "TETRIS", "OLDMAID"] as StudyType[]).map((t) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        else if (t === "TETRIS") { setMaxPlayers(3); setBoardSize(20); }
              ...
              {studyType === "TETRIS" && L(
                <span style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                  <span className="kw">const </span><span className="var">maxPlayers</span><span className="pct"> = </span><span className="num">3</span>
              ...
              {studyType === "OMOK" && L(
                <span style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                  <span className="kw">const </span><span className="var">maxPlayers</span><span className="pct"> = </span><span className="num">2</span>
                  <span className="cmt"> // 1v1 only</span>
                </span>,
                1
              )}
              {studyType !== "OLDMAID" && studyType !== "TETRIS" && studyType !== "OMOK" && L(
                <span style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                  <span className="kw">const </span><span className="var">maxPlayers</span><span className="pct"> = </span>
                  {[2, 3, 4, 5, 6].map((n) => (
                    <button key={n} className={`btn-opt ${maxPlayers === n ? "on" : ""}`} onClick={() => setMaxPlayers(n)} style={{ fontSize: "11px", padding: "3px 8px" }}>
              ...
              {studyType === "OMOK" && L(
                <span style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                  <span className="kw">const </span><span className="var">boardSize</span><span className="pct"> = </span><span className="num">19</span>
              ...
              {studyType === "TETRIS" && L(
                <span style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                  <span className="kw">const </span><span className="var">queue</span><span className="pct"> = </span><span className="str">"20x10"</span>
              ...
              {L(
                <span style={{ display: "flex", gap: "8px" }}>
                  ...
                  <button className="btn-secondary" onClick={() => setShowCreate(false)} style={{ fontSize: "12px" }}>
                    cancel
                  </button>
                </span>,
                1
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ display: "flex", gap: "2px", paddingRight: "8px" }}>
                <span
                  style={{ fontSize: "14px", color: "#555", cursor: "pointer", padding: "0 4px", lineHeight: 1 }}
                  ...
                  {line.type === "cmd" && (
                    <>
                      <span style={{ color: "#6a9955", userSelect: "none" }}>
                        PC024@STUDY-PLATFORM <span style={{ color: "#569cd6" }}>MINGW64</span> <span style={{ color: "#ce9178" }}>~/study-platform</span>
                      </span>
